...
export type EmailTemplateDef = {
  ...
  // Built-in "To" list for a fixed-recipient template. When unset, a fixed
  // template defaults its To to the finance inbox. An admin override in
  // /admin/emails always wins over this. Used by the project-status digest,
  // which ships to a list of founders by default.
  defaultTo?: string[];
...
export const EMAIL_TEMPLATES: EmailTemplateDef[] = [
  ...
  {
    key: "project_status_digest",
    name: "Daily project status digest",
    purpose:
      "A daily portfolio snapshot categorising every project as Running, Planned or Completed, with a short written summary and the projects that need attention.",
    recipient:
      "To: the founders (Thomas, Pascal, Taline, Solene). Add or remove recipients by editing the To / Cc here.",
    trigger: "Automatically once a day at 13:00 UTC (15:00 Paris in summer) via the scheduled digest job.",
    conditions:
      "Sends every day regardless of whether anything changed. The written summary is generated by Claude from the live project + payment data; if Claude is unavailable a plain data-only version is sent instead.",
    toMode: "fixed",
    defaultTo: [
      "thomas.bouquet@htp42.com",
      "pascal.bouquet@htp42.com",
      "taline.nouridjanian-ext@htp42.com",
      "solene.bouquet@htp42.com",
    ],
    defaultCc: [],
    placeholders: [
      { token: "date", description: "The digest date (e.g. 27 August 2026)." },
      { token: "headline", description: "One-line count: running / planned / completed." },
      { token: "summary", description: "The categorised project summary (generated).", block: true },
      { token: "portalUrl", description: "Deep link to the Cockpit." },
    ],
    defaultSubject: "[HTP42] Project status - {{date}}",
    defaultBody: ["{{headline}}", "", "{{summary}}", "", "Full detail in the cockpit: {{portalUrl}}"].join("\n"),
  },
];
