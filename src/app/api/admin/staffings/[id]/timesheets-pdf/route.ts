...
// Covers the officially-logged lifecycle (Submitted / Invoiced / Paid);
// Draft and Deleted are left out. Returns the PDF as a direct download.
    ...
    const pdf = await generateTimesheetSummaryPdf(
      {
        ...
        consultant: `${first.memberName || first.memberCode}${
          first.memberCode && first.memberName ? ` (${first.memberCode})` : ""
        }`,
      },
      rows,
    );
